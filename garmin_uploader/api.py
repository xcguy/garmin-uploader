...
URL_ACTIVITY_BASE = (
    "https://connect.garmin.com/modern/proxy/activity-service/activity"  # noqa
)
...
class GarminAPIException(Exception):
    """
    An Exception occurred in Garmin API
    """
...
class GarminAPI:
    ...
    def authenticate(self, username, password):
        ...
        Try to mimic a browser behavior trying to login
        ...
        if not resp.ok:
            raise Exception(
                "Invalid SSO first request status code {}".format(resp.status_code)
            )  # noqa
        ...
        regex = (
            "var response_url(\s+)= (\"|').*?ticket=(?P<ticket>[\w\-]+)(\"|')"  # noqa
        )
        params = {}
    ...
    def upload_activity(self, session, activity):
        ...
        if res.status_code not in (200, 201, 409):
            if res.status_code == 412:
                logger.error(
                    "You may have to give explicit consent for uploading files to Garmin"  # noqa
                )
    ...
    def set_activity_name(self, session, activity):
        ...
        if not res.ok:
            raise GarminAPIException(
                "Activity name not set: {}".format(res.content)
            )  # noqa
    ...
    def load_activity_types(self):
        ...
        logger.debug(
            "Fetched {} activity types".format(len(self.activity_types))
        )  # noqa
        return self.activity_types
    ...
    def set_activity_type(self, session, activity):
        ...
        if not res.ok:
            raise GarminAPIException(
                "Activity type not set: {}".format(res.content)
            )  # noqa
